...
def _get_rgbdgsd_pairs(folder, mode='train'):
    img_paths = []
    mask_paths = []
    if mode == 'train':
        ...
        mask_folder = os.path.join(folder, 'test/masks')
    else:
        assert  mode == "test"
        img_folder = os.path.join(folder, 'test/images')
        mask_folder = os.path.join(folder, 'test/masks')
