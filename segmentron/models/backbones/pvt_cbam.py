...
class ChannelAttention(nn.Module):
    def __init__(self,in_chans,ratio=16):
        ...
        self.fc1 = nn.Conv2d(in_chans, in_chans//ratio , 1 , bias=False)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Conv2d(in_chans//ratio, in_chans,  1 ,bias=False)
...
class CBAM(nn.Module):
    def __init__(self, in_chans, ratio=16, kernel_size=7):
        super(CBAM, self).__init__()
        self.ca = ChannelAttention(in_chans, ratio)  # 通道注意力实例
        self.sa = SpatialAttention(kernel_size)  # 空间注意力实例

    def forward(self, x):
        out = x * self.ca(x)  # 使用通道注意力加权输入特征图
        result = out * self.sa(out)  # 使用空间注意力进一步加权特征图
        return result + x  # 返回最终的特征图
#-----------------------------CBAM---------------------------------#
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.block_cbam1=CBAM(64)
        self.block_cbam2=CBAM(128)
        self.block_cbam3=CBAM(320)
        self.block_cbam4=CBAM(512)
