            ...
            if iteration % self.iters_per_epoch == 0:
                ...
                if current_mIoU > self.best_mIoU:
                    ...
                    if self.save_to_disk:
                        logging.info("=> Epoch {}: Found new best mIoU: {:.3f}".format(epoch, self.best_mIoU * 100))
                if self.save_to_disk:
                    save_checkpoint(self.model, epoch, self.optimizer, self.lr_scheduler, is_best=is_best)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        trainer.train()
        best_model_path = os.path.join(cfg.TRAIN.MODEL_SAVE_DIR, 'best_model.pth')
        
        if os.path.isfile(best_model_path):
            logging.info('Training finished. Loading best model from {} for testing...'.format(best_model_path))
            ...
            logging.warning('Best model not found at {}, using last checkpoint for testing.'.format(best_model_path))
        trainer.test()